...
def fix_maze_conditions_consistency(sessions: List[Data]) -> List[Data]:
    ...
    # Pick reference session
    condition_counts = [len(np.unique(s.trials.maze_condition)) for s in valid_sessions]
    ref_idx = condition_counts.index(max(condition_counts))
    reference_session = valid_sessions[ref_idx]

    # Build reference group signatures
    ref_conditions = sorted(np.unique(reference_session.trials.maze_condition))
    ref_groups = [ref_conditions[i:i + 3] for i in range(0, len(ref_conditions), 3)]
    ref_group_signatures = {
        idx: get_group_signature(reference_session, group)
    ...
    for session in valid_sessions:
        ...
        for group in session_groups:
            ...
            if matched_ref_group_idx is None:
                raise ValueError(
                    f"Could not match group {group} in session {session.session.id}"
                )
            new_base = matched_ref_group_idx * 3 + 1
            for i, old in enumerate(sorted(group)):
                condition_mapping[old] = new_base + i
    ...
    # Return sessions plus reference summary/plot for context
    summary_df, fig = analyze_maze_conditions(reference_session)
    print("Summary of maze conditions:")
    display(summary_df)
    fig.show()
...
def load_sessions(
    data_path: Path,
    subject_name: str,
    num_files_to_load: Union[int, str] = "auto",
) -> List[Data]:
    """Load, clean, and harmonise MC Maze sessions from HDF5 files."""
    subject_name = subject_name.lower()
    defaults = {"jenkins": 4, "nitschke": 3}
    if num_files_to_load == "auto":
        num_files_to_load = defaults.get(subject_name, 3)

    h5_files = [f for f in os.listdir(data_path) if f.endswith(".h5")]
    subject_files = [f for f in h5_files if subject_name in f.lower()]

    if not subject_files:
        raise FileNotFoundError(f"No files found for subject {subject_name}")

    if len(subject_files) < num_files_to_load:
