...
def analyze_discrete_variable(
    ...
    """Analyzes a discrete variable to find feature selectivity."""
    results = []
    ...
    for value in unique_values:
        ...
        in_df = event_acts_df.groupby(["instance_idx", "feature_idx"]).agg(
        ...
        out_acts_df = acts_df[out_mask].merge(
            promising[["instance_idx", "feature_idx"]],
            on=["instance_idx", "feature_idx"], how="inner"
        )

        if not out_acts_df.empty:
            out_df = out_acts_df.groupby(["instance_idx", "feature_idx"]).agg(
                activation_count=("activation_value", "count")
            ).reset_index()
            n_out = len(metadata_binned) - n_in
            out_df["activation_frac_outside"] = out_df["activation_count"] / n_out
            merged = promising.merge(out_df, on=["instance_idx", "feature_idx"], how="left")
...
def map_features_to_metadata(
    ...
    """Automatically maps features to metadata variables, returning a ranked DataFrame of associations."""
    ...
    print(f"\nRanking features and selecting top {top_n_features} for each variable...")
    ranked_df = (
...
def plot_feature_tuning(
    ...
    if 'angle' in variable:  # Polar plot
        ...
        fig = make_subplots(
            ...
            subplot_titles=["Feature Activation", "Top Unit Z-score", "Bottom Unit Z-score", "Selectivity Score", "Mean Z-scores when Feature Active"]
        )
        ...
        fig = make_subplots(rows=2, cols=4, specs=[[{}, {}, {}, {}], [{"colspan": 4}, None, None, None]], subplot_titles=["Feature Activation", "Top Unit Z-score", "Bottom Unit Z-score", "Selectivity Score", "Mean Z-scores when Feature Active"], horizontal_spacing=0.1, vertical_spacing=0.3)
...
def build_feature_finding_dashboard(
    feature_metadata_mapping: pd.DataFrame,
    acts_df: pd.DataFrame,
    spikes_df: pd.DataFrame,
    metadata_binned: pd.DataFrame,
):
    """Builds an interactive dashboard for exploring feature-to-metadata associations."""
    ...
    mode_radio = widgets.RadioButtons(
        options=[('Preset (from feature metadata mapping table)', 'preset'),
                 ('Manual selection', 'manual')],
        value='preset', description=''
    )

    # Ensure feature_metadata_mapping has 'bvar'
    if 'bvar' not in feature_metadata_mapping.columns:
        feature_metadata_mapping['bvar'] = feature_metadata_mapping.apply(
            lambda r: _bvar_name(r['variable'], r['variable_type']), axis=1
        )

    # Preset dropdown
    preset_entries = []
    for _, r in feature_metadata_mapping.iterrows():
        bvar = r['bvar']
    ...
    feature_dropdown = widgets.Dropdown(description='Feature:', options=[])
    ...
    def _refresh_variable_options(*_):
        ...
        mask = (feature_metadata_mapping['instance_idx'] == inst)
        vars_for_inst = sorted(feature_metadata_mapping.loc[mask, 'bvar'].unique())
    ...
    def _refresh_feature_options(*_):
        ...
        mask = (feature_metadata_mapping['instance_idx'] == inst) & (feature_metadata_mapping['bvar'] == sel_var)
        feats = sorted(feature_metadata_mapping.loc[mask, 'feature_idx'].unique())

        prev_feat = feature_dropdown.value
        feature_dropdown.options = feats
        feature_dropdown.value = prev_feat if prev_feat in feats else (feats[0] if feats else None)
